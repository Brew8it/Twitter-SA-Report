...
\begin{lstlisting}[style=mypython,caption={Avlägsna HTML-kod.},label=pre:html]
...
Omvandling från versaler till gemener utförs på varje twitterinlägg m.h.a den inbyggda Python-funktionen \pyth{str.lower()} som konverterar versaler till gemener.
\begin{lstlisting}[style=mypython,caption={Omvandla versaler till gemener.},label=pre:lower]
def to_lower(self):
      self.df["tweet"] = self.df["tweet"].str.lower()
\end{lstlisting}

*** Skriv om expand cont *****
\begin{lstlisting}[style=mypython,caption={Expandera sammandragsförkortningar},label=pre:expand]
...
\begin{lstlisting}[style=mypython,caption={Avlägsna accent.},label=pre:accent]
...
För att avlägsna URL, “mentions”, “hashtags” , skiljetecken, siffror, emojis och repeterade mellanslag används fyra reguljära uttryck (RU) \cite{impl:003}. För att avlägsna URL används \pyth{'https?://[A-Za-z0-9./]+'} vilket betyder att om en URL är http eller https kommer denna avlägsnas, samt om den efterföljs av ännu en URL kommer även den tas bort.
...
\begin{lstlisting}[style=mypython,caption={Avlägsna URL:er, "mentions", “hashtags”, skiljetecken, siffror, emojis och repeterande mellandlsag.},label=pre:rem]
...
För att utföra ordstamsigenkänning och avlägsning av stoppord måste varje twitterinlägg först tokeniseras, vilket betyder att varje ord i ett twitterinlägg blir ett eget element i en lista. Tokeniseringen utförs m.h.a funktionen \pyth{word_tokenize()} \cite{impl:006} från Natural Language Toolkit (NLTK).
\begin{lstlisting}[style=mypython,caption={Tokenisera twitterinlägg till listor av ord.},label=pre:token]
def tokenize(self):
      self.df["tweet"] = self.df["tweet"].apply(word_tokenize)
\end{lstlisting}

Ordstamsigenkänning utförs m.h.a \pyth{SnowballStemmer('english').stem()} \cite{impl:008} från NLTK som försöker hitta varje ords ordstam vilket diskuteras i avsnitt \ref{exp:pre}.
\begin{lstlisting}[style=mypython,caption={Ordstamsigenkänning.},label=pre:stem]
def word_stemming(self):
       self.df["tweet"] = self.df["tweet"]./
       apply(lambda x: [SnowballStemmer('english').stem(y) for y in x])
\end{lstlisting}

Avlägsning av stoppord utförs m.h.a \pyth{stopwords.word('english')} \cite{impl:007} från NLTK som är en lista av engelska stoppord. \pyth{remove_stopwords(self)} kontrollerar om något ord i twitterinlägget finns med i listan, om det gör det tas det bort. 
\begin{lstlisting}[style=mypython,caption={Avlägsna stoppord.},label=pre:stop]
