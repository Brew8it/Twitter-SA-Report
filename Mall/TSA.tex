...
Detta kapitel beskriver detaljer angående implementationen av vår studie och dess flöde som illustrerats i figur \ref{fig:overfig}. Kapitlet beskriver även teknikerna som används för att göra implementationen möjlig. Gemensamma tekniker som används i alla delar av implementation är programmeringsspråket Python3 \cite{impl:001} och Pandas \cite{impl:002}. Pandas är ett Python paket som erbjuder snabba, flexibla datastrukturer som är utformade för att göra arbetet med relation och märkt data smidigare. 
\subsection{Databearbetning}\label{impl:pre}
Som beskrivs i avsnitt \ref{exp:pre} är databearbetning ett viktigt steg inom SA och TSA. I detta avsnittet beskrivs implementationen av databearbetningen m.h.a koddelarna som illustreras i figur \ref{fig:dataset}.

För att avlägsna HTML-kod från twitterinlägg använder vi oss utav BeautifulSoup \ref{impl:004}, som är ett Python-paket som extrahera data från HTML-kod.
Om HTML-kod återfinns i twitterinlägg tas det således bort. 
\begin{python}
...
För att avlägsna URL, “mentions”, “hashtags” , skiljetecken, siffror och repeterade mellanslag används fyra reguljära uttryck (RU) \cite{impl:003}. För att avlägsna URL används \pythoninline{r'https?://[A-Za-z0-9./’]+'} vilket betyder att om en URL är http eller https kommer denna avlägsnas, samt om den efterföljs av ännu en URL kommer även den tas bort.

“Mentions” avlägsnas med det RU \pythoninline{r'@[A-Za-z0-9]+'}, d.v.s om en alfanumerisk sträng börjar med ‘@’ avlägsnas den alfanumeriska strängen.

Avlägsning av “hashtags”, skiljetecken och siffror görs med det RU \pythoninline{'[^a-zA-Z]'}, d.v.s en sträng måste börja på bokstäver mellan a-z, om inte tas dessa bort.
...
Avlägsning av stoppord utförs m.h.a \pythoninline{stopwords.word(‘english’)} \cite{impl:007} från NLTK som är en lista utav engelska stoppord. \pythoninline{remove_stopwords(self)} kollar är om något ord i twitterinlägget finns med i listan, om det gör det tas detta bort. 
\begin{python}
