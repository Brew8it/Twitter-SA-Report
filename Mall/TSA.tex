...
Syftet med studien är att utvärdera (x) stycken maskininlärningsalgoritmer och hur bearbetningen samt märkningen av en datamängd påverkar algoritmens prestanda. De (x) algoritmerna är Naive Bayes, Support Vector Machine och ZZZZZ. Figur \ref{fig:overfig} visar en förenklad överblick hur processen för att utvärdera algoritmerna kommer se ut. 

\begin{figure}[h]
\includegraphics[width=12cm]{oversiktsfigur}
\centering
\caption{Utvärderingsprocessen av maskininlärningsalgoritmer}
\label{fig:overfig}
\end{figure}

Studien är av intresse för vår uppdragsgivare, CGI, för de betraktar sentimentanalys som en viktig pusselbit för framtida lösningar dem vill erbjuda sina kunder. Lösningar skulle kunna vara chatbotar där sentimentanalys används så att chatboten ändrar sitt språk utefter svaren från slutanvändare. Eller att det kan användas för trendanalys där ett företag vill veta vad allmänheten tycker före och efter att man har släppt en ny produkt eller efter att kvartalsrapporten har släppts. 

I avsnitt \ref{SA} förklaras vad sentimentanalys är och under \ref{SAkt} beskrivs dem olika klassificeringsteknikerna inom SA.
...
Övervakad inlärning används när datamängden består av både indata och dess förväntade utdata. Den övervakade algoritmen använder datamängden för att lära sig hur indata beror på utdata genom att skapa en klassificeringsmodell som används för att förutse utdata från ny indata som illustreras i figur \ref{fig:kl}. 
...
Övervakad inlärning kan brytas ner till klassificerings- och regressionsproblem och eftersom TSA kan kallas ett typiskt klassificeringsproblem \cite{SAsurvey}, kommer vår studie att använd algoritmer lämpade för sådana problem. I figur \ref{fig:ml} kan vi se några utav dessa klassificeringsalgoritmer och enligt \cite{TSAsurvey} är speciellt Naive Bayes (NB) och Support Vector Machine (SVM) algoritmerna bäst lämpade för TSA.
...
Det finns tre varianter utav Naive Bayes nämligen Gaussian, Multinomial och Bernoulli. Gaussian används när attributen är kontinuerligt fördelade och då antas det att attributen förknippade med varje klass fördelas enligt Gaussian d.v.s normalfördelning \cite{nb:002}. Bernoulli användas när attributen är fördelade enligt multivariat Bernoulli \cite{nb:004} och Multinomial NB används när attributen är multinomial fördelat. Enligt \cite{nb:003} används Multinomial NB för klassificering av text och detta gör att Multinomial NB passar perfekt för TSA.
...
\begin{figure}[h]
\includegraphics[width=12cm]{fm}
\centering
\caption{Förvirringsmatris för utvärdering av klassificeringsmodeller.}
\label{fig:fm}
\end{figure}
\section{Utvärdering} \label{TSAev}
I figur \ref{fig:fm} ser vi en så kallad förvirringsmatris (FM) som utvärderar en klassificeringsmodell från datamängden där “positivt” eller “negativt” är förbestämt. Matrisen visar antalet sann positiva (SP), sann negativ (SN), falsk positiv (FP) och falsk negativ(FN) \cite{wiki:003}. Med dessa värden kan vi jämföra och analysera modellerna m.h.a följande utvärderingsmetoder: noggrannhet (n) \cite{wiki:011}, precision (p),  återkallelse (å) \cite{wiki:002} och F-Score \cite{wiki:001}.
