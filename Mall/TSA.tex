...
En viktig del inom SA är bearbetningen av den data som ska analyseras. Som nämndes i avsnitt \ref{TSAchall}, innehåller twitterinlägg stora mängder brus på grund av det informella språket som används på Twitter, dvs data som inte är användbar för analysprocessen. Kvaliteten på datan som ska analyseras är avgörande för vilka resultat som kan uppnås vid maskininlärningen \cite{effrosynidis2017comparison}. Därför behöver data som ska analyseras bearbetas innan maskininlärningen, vilket betyder att data normaliseras och rensas på brus. Exempelvis kan det bestå av att termer som inte är nödvändiga tas bort, ersätts eller slås samman med andra. 
...
För att ekvation \ref{eq:bt} skall vara “Naive” antar vi att varje ord $b_w$ i $B$ är oberoende av varandra vilket betyder att vi inte längre klassificerar $B$ utan $b_w$, $P(B|A) = P(b_{w1}|A) * P(b_{w2}|A) * … * P(b_{wn}|A)$ där $n$ är antalet ord i $B$.

För att beräkna sannolikheten att $b_w$ är $A$ används en Dirichlet prior \cite{wiki:012, nb:007} där $N_{b_w}$ är hur många gånger $b_w$ förekommer i delmängden av data för $A$, $N_A$ är summan av alla ord i delmängden för $A$, $\alpha$ och $\alpha_n$ används för att utföra additiv utjämning \cite{wiki:013} där $\alpha = 1$ och $\alpha_n$ är summan antal möjliga ord som existerar i hela datamängden, 

\begin{equation}
P(w_b|A)= \frac{N_{b_w} + \alpha}{N_A + \alpha_n}.
\end{equation}

I tabell \ref{tab:dataset} illustreras en datamängd med fyra twitterinlägg och dess märkning, denna datamängden kan användas för att beräkna om twitterinlägget “solen skiner alltid i karlstad” är positiv eller negativ. Genom att räkna ut sannolikheten $P(Positiv|solen skiner alltid i karlstad)$ och $P(Negativ|solen skiner alltid i karlstad)$ kan märkning bestämmas, detta görs genom att jämföra vilken sannolikhet som blir störst. Och eftersom vi ska jämföra uträkningarna mellan varandra kan divisionen med $P(B)$ strykas ur bägge uträkningarna och vi för följande ekvationer,

\begin{equation}\label{eq:nbpos}
P(Positiv|solen skiner alltid i karlstad) = P(solen|Positiv) * P(skiner|Positiv) * P(alltid|Positiv) * P(i|Positiv) *  P(karlstad|Positiv) * P(Positiv)
\end{equation}

\begin{equation}\label{eq:nbpos}
P(Negativ|solen skiner alltid i karlstad) = P(solen|Negativ) * … * P(karlstad|Negativ) * P(Negativ)
\end{equation}
...
Eftersom antalet särdrag som genereras vid klassificering av text är så pass högt, resulterar det i att en icke-linjär klassificerare överanpassar data \cite{svm:005}. Därför har en linjär klassificerare valts att användas vid den här studien.
