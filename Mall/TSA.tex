...
Syntaktiska särdrag används för att utforska påverkan av olika termer i SA och TSA. Syntaktiska särdrag bryts ner till unigram, bigram, trigram, n-gram, termfrekvens och “Part of Speech” (POS), där n-gram är ett samlingsord för uni-, bi- och trigram. I figur \ref{fig:grams} illustreras hur n-grammodellen fungerar på en enkel mening och hur meningens ord grupperas på olika sätta till ett ord eller fras. **ta upp något om varför det används?**
...
Studien kommer implementera n-gram och termfrekvens för varje klassificeringsalgoritm i avsnitt \ref{impl}. POS kommer inte implementeras i studien då \cite{go2009twitter, feature:001} rapporterar försämrad precision. Däremot rapporteras i \cite{feature:002} små förbättringar vid användning av POS.
...
I \ref{MLnb} beskrivs att Naive Bayes-algoritmen är baserad på Bayes theorem  \cite{wiki:009} vilket för TSA betyder att räkna ut sannolikheten att ett twitterinlägg $B$ är positivt eller negativt $A$. Där $P(A | B)$ är en villkorlig sannolikhet för att $A$ inträffar givet att $B$ är sant, $P(B | A)$ är också en villkorlig sannolikhet för att $B$ inträffar givet att $A$ är sant. $P(A)$ och $P(B)$ är sannolikheten att observera $P(A)$ och $P(B)$ oberoende av varandra,
...
För att beräkna sannolikheten att $b_w$ är $A$ används Dirichlet prior \cite{wiki:012, nb:007}, där $N_{b_w}$ är hur många gånger $b_w$ förekommer i delmängden av data för $A$, $N_A$ är summan av alla ord i delmängden för $A$, $\alpha$ och $\alpha_n$ används för att utföra additiv utjämning \cite{wiki:013} där $\alpha = 1$ och $\alpha_n$ är summan av antal olika ord i datamängden, 
...
I tabell \ref{tab:dataset} illustreras en datamängd med fyra twitterinlägg och deras märkning. Denna datamängd kan användas för att beräkna om twitterinlägget “solen skiner alltid i karlstad” är positivt eller negativt. Genom att räkna ut sannolikheten $P(Positiv|\textrm{solen skiner alltid i karlstad})$ och $P(Negativ|\textrm{solen skiner alltid i karlstad})$ kan märkning bestämmas, detta görs genom att jämföra vilken sannolikhet som blir störst. Och eftersom vi ska jämföra uträkningarna mellan varandra kan divisionen med $P(B)$ strykas ur bägge uträkningarna och vi för följande ekvationer,
\begin{multline*}
...
För att avlägsna HTML-kod från twitterinlägg använder vi oss av BeautifulSoup \cite{impl:004}, som är ett Python-paket för att extrahera data från HTML-kod.
...
Omvandling från versaler till gemener utförs på varje twitterinlägg m.h.a den inbyggda Python-funktionen \pythoninline{str.lower()} som konverterar versaler till gemener.
\begin{python}
def to_lower(self):
...
def expand_contractions(self):
       self.df["tweet"] = self.df["tweet"]./
       apply(lambda x: self.expand_cont(x))
\end{python}

Avlägsna accenter utförs genom att använda \pythoninline{unicodedata.normalize()} \cite{impl:005} som använder “Normalization Form Compatibility Decomposition” (NFKD) parametern,  vilket betyder att \pythoninline{unicodedata.normalize()} byter ut en bokstav med accent mot sin ASCII-ekvivalenta bokstav. Resterande parametrar säger att om bokstaven redan är ascii eller utf-8 ignoreras dessa bokstäver. 
...
För att avlägsna URL, “mentions”, “hashtags” , skiljetecken, siffror, emojis och repeterade mellanslag används fyra reguljära uttryck (RU) \cite{impl:003}. För att avlägsna URL används \pythoninline{'https?://[A-Za-z0-9./]+'} vilket betyder att om en URL är http eller https kommer denna avlägsnas, samt om den efterföljs av ännu en URL kommer även den tas bort.

“Mentions” avlägsnas med det RU \pythoninline{'@[A-Za-z0-9]+'}, d.v.s om en alfanumerisk sträng börjar med ‘@’ avlägsnas den alfanumeriska strängen.

Avlägsning av “hashtags”, skiljetecken, siffror och emojis görs med det RU \pythoninline{'[^a-zA-Z]'}, d.v.s en sträng måste börja på bokstäver mellan a-z, om inte tas dessa bort.

Och slutligen så avlägsnas repeterande mellanslag, vilket utförs med det RU \pythoninline{' +'}. Ifall det är två eller fler repeterande mellanslag tas dessa bort.
...
För att utföra ordstamsigenkänning och avlägsning av stoppord måste varje twitterinlägg först tokeniseras, vilket betyder att varje ord i ett twitterinlägg blir ett eget element i en lista. Tokeniseringen utförs m.h.a funktionen \pythoninline{word_tokenize()} \cite{impl:006} från Natural Language Toolkit (NLTK).
\begin{python}
def tokenize(self):
      self.df["tweet"] = self.df["tweet"].apply(word_tokenize)
\end{python}

Ordstamsigenkänning utförs m.h.a \pythoninline{SnowballStemmer('english').stem()} \cite{impl:008} från NLTK som försöker hitta varje ords ordstam vilket diskuteras i avsnitt \ref{exp:pre}.
\begin{python}
def word_stemming(self):
       self.df["tweet"] = self.df["tweet"].
       apply(lambda x: [SnowballStemmer('english').
       stem(y) for y in x])
\end{python}

Avlägsning av stoppord utförs m.h.a \pythoninline{stopwords.word('english')} \cite{impl:007} från NLTK som är en lista av engelska stoppord. \pythoninline{remove_stopwords(self)} kontrollerar om något ord i twitterinlägget finns med i listan, om det gör det tas det bort. 
\begin{python}
