...
I \ref{bak:NB} beskrivs det att Naive Bayes algoritmen är baserad på Bayes theorem  \cite{ref:wiki:009} vilket för TSA betyder att räkna ut sannolikheten om ett twitterinlägg $B$ är positivt $A$ eller negativt $!A$. 
\begin{equation}\label{eq:btpos}
p(A|B) = \frac{p(B|A) * p(A)}{P(B)}
\end{equation}
\begin{equation}\label{eq:bt}
p(!A|B) = \frac{p(B|!A) * p(!A)}{P(B)}
\end{equation}\label{eq:btneg}

Som nämns ovan är TSA ett binärt klassificeringsproblem d.v.s om  $B$ är $A \lor !A$ . Därför delas ekvation \ref{eq:btpos} med ekvation \ref{eq:btneg} vilket ger oss $\hat{P} > 1 = A$ och om $\hat{P} < 1 = !A$

\begin{equation}
\hat{P} = \frac{p(A|B) = p(B|A) * p(A)}{p(!A|B) = p(B|!A) * p(!A)}
\end{equation}

För att ekvation \ref{eq:phat} skall vara “Naive” antar vi att varje ord $b_w$ i $B$ är oberoende av varandra vilket betyder att vi inte längre klassificerar på $B$ utan på $b_w$, $P(B) = P(b_{i1}) * P(b_{i2}) * … * P(b_{in})$ där $n$ är antal order i $B$.
